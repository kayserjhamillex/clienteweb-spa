...
@Component({
  selector: 'app-registerapptemplate',
  templateUrl: './registerapptemplate.component.html',
  styleUrls: ['./registerapptemplate.component.css']
})
export class RegisterapptemplateComponent implements OnInit {
  usuario: SocialUser;
  loggedIn: boolean;
  editarnombre;
  cliente1: Cliente = {
  ...
  validar() {
    ...
    const parametro = parseInt(this.activatedRoute.snapshot.paramMap.get('id'));
    ...
    if (this.tienecuenta === false) {
      ...
      this.clienteService.savePersona(this.persona).subscribe(
        res => {
          ...
          const parametrito = parametro.toString();
          this.toastr.success('ese le envio un correo para confirmar su registro');
          this.reservaService.getSendconfirmation(mail, codigo, parametrito).subscribe(
